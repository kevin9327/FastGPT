...
checkValue(op, target, storedValue, tagType): boolean
  ...
  array：
    $is      -> 两个数组去重后集合相等
    $isNot   -> 两个数组去重后集合不相等
    $contains -> 目标为单字符串时存储数组包含目标字符串；目标为数组时存储数组必须包含目标数组的所有项（$all 语义）
    $notContains -> 目标为单字符串时存储数组不包含目标字符串；目标为数组时存储数组不能包含目标数组的任何一项（完全排除语义）
    $in      -> 存储数组是目标数组的子集
    $notIn   -> 存储数组不是目标数组的子集
  string：
    $eq/$ne 使用大小写敏感比较
...
2. 承载记录选项回填：整理 `fromMigration: true` 承载标签时，读取该知识库旧表 `dataset_collection_tags` 中的全部历史标签名称，以及各 Collection 上已打在该承载标签上的全部已用值，合并去重后写入 `carrier.options`。
3. 从 v1 表建立 `旧 tagId -> 标签名称` 映射；遇到旧字符串标签时，按需创建或复用当前 dataset 的系统承载标签并合并名称数组；创建承载记录时以旧表标签作为初始预设 `options`，并将转换出的标签名称实时合并追加至 `carrier.options`。
4. 保留其他新格式标签，移除旧字符串元素，使用原始 BSON 快照条件更新。
5. 建立 v2 唯一索引和 `tags.tagId` 索引，校验无旧字符串标签、无重复定义、无重复或非法承载标签。
...
- [x] 承载记录选项回填：在 `ensureDatasetTagMigrationCarrier` 创建承载记录时以旧表标签初始化预设 `options`。
- [x] 承载记录选项合并：在 `migrateDatasetTagDefinitions` 中合并旧表定义与 Collection 已打标的值至 `carrier.options`。
- [x] 集合值迁移同步：在 `migrateCollectionTagValues` 中同步将 `migratedValues` 通过 `$addToSet` 追加至 `carrier.options`。
- [x] 运行时兼容更新：在 `createOrGetCollectionTags` 解析到旧格式 `defaultValues` 时同步更新 `defaultTag.options`。
- [x] 补充与更新单测，覆盖上述 options 回填、合并和去重场景。
- [x] 运行局部测试、App typecheck、相关 lint 和 `git diff --check` 验证。
